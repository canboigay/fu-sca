...
class WebProxy:
    ...
    def __init__(self, starting_url: str, logger, *, browser: str = "webkit", headless: bool = True, safe: bool = False):
        """
        ...
        Args:
            ...
            browser: one of 'webkit', 'chromium', 'firefox'
        ...
        self.request_map = {}
        self.browser_choice = (browser or "webkit").lower()
        if self.browser_choice not in ("webkit", "chromium", "firefox"):
            self.browser_choice = "webkit"
    ...
    def create_proxy(self):
        ...
        Stores references on the instance for recovery.
        ...
        # Select engine
        launcher = getattr(playwright, self.browser_choice)
        # Basic retry if launch fails
        last_err = None
        for _ in range(2):
            try:
                ...
                last_err = e
        if last_err and 'browser' not in locals():
            raise last_err
        ...
        # Safe mode: inject guards to block non-GET requests via fetch/XHR
        if self.safe:
            guard_js = """
            (() => {
              ...
              window.fetch = async function(input, init) {
                ...
                if (method !== 'GET') {
                  console.warn('[SAFE_MODE] Blocked fetch with method', method);
                  throw new Error('[SAFE_MODE] Blocked non-GET fetch: ' + method);
                }
              ...
              XHR.prototype.send = function(body) {
                const m = this.__method || 'GET';
                if (m !== 'GET') {
                  console.warn('[SAFE_MODE] Blocked XHR with method', m);
                  throw new Error('[SAFE_MODE] Blocked non-GET XHR: ' + m);
                }
                return sendOrig.apply(this, arguments);
              };
            })();
            """
            page.add_init_script(guard_js)
        
        # Setup monitoring
        self.setup_monitoring(context, page)
        
        # Stash references for recovery
    ...
    def ensure_page(self):
        """Ensure we have a valid, open page. Recreate if needed and return page.
        """
        try:
            # Try a lightweight call to verify context
            _ = self.context.pages
            _ = self.page.title()
            return self.page
        except Exception:
            # Recreate browser/context/page
            try:
                ...
                pass
            # Relaunch
            launcher = getattr(self.playwright, self.browser_choice)
