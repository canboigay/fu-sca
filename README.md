...
> Advanced LLM-powered security testing platform with:
> - DeepSeek API support (default) with OpenAI fallback
> - WebKit browser engine optimized for macOS ARM
> - Context-aware vulnerability discovery and validation
> - Professional-grade reporting and analysis

[![License: GPL-3.0](https://img.shields.io/badge/License-GPL%203.0-blue.svg)](https://www.gnu.org/licenses/gpl-3.0)
[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)

FU-SCA is an advanced AI-powered security testing platform that leverages Large Language Models to intelligently discover and validate web application vulnerabilities. Unlike traditional scanners that follow predefined patterns, this tool employs adaptive reasoning - analyzing application behavior, generating sophisticated test cases, and validating findings through autonomous decision-making.

Built for professional security researchers and bug bounty hunters seeking to leverage cutting-edge AI for vulnerability discovery.
...
- Playwright with WebKit browser engine

### Installation

```bash
# Clone the repository
git clone https://github.com/canboigay/fu-sca
cd fu-sca
...
# Install Playwright browsers (WebKit)
...
Safe (read-only) reconnaissance:
```bash
python run.py -u https://example.com --safe --browser webkit --headless
```

```bash
# Basic scan of a single URL (DeepSeek is used automatically if DEEPSEEK_API_KEY is set)
...
This repo is tested regularly with Playwright 1.57.x. If you see unexpected browser crashes, run `bash scripts/setup_playwright.sh` to reinstall browser binaries and prefer `--browser webkit` on macOS ARM.

### Model Selection

By default this fork prefers DeepSeek models when `DEEPSEEK_API_KEY` is set.

- deepseek-chat (general)
- deepseek-reasoner (chain-of-thought style)

If `OPENAI_API_KEY` is set instead, it falls back to:
...
- [ ] Wrap JS snippets for Playwright evaluate automatically (no bare `return`)
- [ ] Add dynamic form discovery to avoid hard-coded selectors
- [ ] Add error recovery for timeouts and async/sync misuse
- [ ] Minimal test suite to import modules and validate CLI args
- [ ] Optional: Async Playwright migration
...
- OpenAI and DeepSeek for their powerful language models
