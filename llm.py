...
class LLM:
    ...
    def __init__(self, knowledge_content: Optional[str] = None):
        ...
        """
        # Try DeepSeek first (cheaper), then OpenAI
        ...
        if deepseek_key:
            ...
            self.model_reasoning = "deepseek-reasoner"
            self.model_chat = "deepseek-chat"
            print("[Info] Using DeepSeek API")
        elif openai_key:
            self.client = OpenAI(api_key=openai_key)
            self.model_reasoning = "o4-mini"
            self.model_chat = "gpt-4o"
            print("[Info] Using OpenAI API")
        else:
            raise ValueError("No API key found. Set DEEPSEEK_API_KEY or OPENAI_API_KEY environment variable.")
        ...
        - JavaScript: NEVER use a bare top-level `return` in code for page.evaluate. Prefer `() => expression` or a function that returns a value.
        - Selectors: Always verify an element exists before click/fill/submit. If unsure, request `discover_forms` or use provided DISCOVERED_FORMS_JSON.
        - Python: Do NOT attempt to launch Playwright; use the provided `page` context only.
        - Actions: One atomic action per step; avoid repeating failed actions without modification.
        - Safe Mode: If enabled, do NOT propose click/fill/submit or any action that triggers non-GET network requests. Focus on navigation, DOM analysis, headers, JS endpoints, and forms.
    ...
    def reason(self, messages: List[Dict[str, str]], reasoning: str = "medium") -> str:
        ...
        """
        # DeepSeek doesn't support reasoning_effort parameter
        if hasattr(self, 'model_reasoning') and 'deepseek' in self.model_reasoning:
