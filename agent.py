...
class Agent:
    ...
    def __init__(self, starting_url: str, expand_scope: bool = False, 
                 ...
                 browser: str = 'webkit', headless: bool = True, timeout_ms: int = 7000,
                 safe: bool = False):
        """
        ...
        self.proxy = WebProxy(starting_url, logger, browser=self.browser, headless=self.headless, safe=self.safe)
        ...
        self.scanner = None
        self.tools = Tools(default_timeout_ms=self.timeout_ms, safe=self.safe)
        self.history = []
        self.reporter = Reporter(starting_url, output_dir=self.output_dir)
    ...
    def run(self):
        ...
        # Always derive page from proxy to allow recovery
        page = self.proxy.page
        ...
        while urls_to_parse:
            ...
            # Ensure page is alive before scanning
            page = self.proxy.ensure_page()
            self.scanner = Scanner(page)
            ...
            # Include discovered forms to avoid selector guessing
            try:
                forms_json = self.tools.discover_forms(page)
            except Exception:
                forms_json = "[]"
            page_data = (
                f"Page information: {page_source}\n"
                f"*** URL of the page we are planning for: {url} ***\n"
                f"*** DISCOVERED_FORMS_JSON: {forms_json} ***"
            )
        ...
        # If safe mode, emit safe crawl artifacts (JSON/MD)
        if self.safe:
            try:
                forms_json = self.tools.discover_forms(page)
            except Exception:
                forms_json = "[]"
            logger.info("Writing safe crawl artifacts", color='yellow')
            # include_third_party and export_csv are derived from CLI flags stored on Agent via args? Pass via environment to keep minimal.
            include_third_party = os.getenv('ROGUE_INCLUDE_THIRD_PARTY', 'false').lower() in ('1','true','yes')
            export_csv = os.getenv('ROGUE_EXPORT_CSV', 'false').lower() in ('1','true','yes')
            self.reporter.generate_safe_crawl_summary(self.proxy, forms_json, include_third_party=include_third_party, export_csv=export_csv)
    ...
    def _execute_single_plan(self, plan: dict, page, plan_index: int, total_plans: int) -> str:
        ...
        while iterations < self.max_iterations:
            ...
            # Auto-recover if browser/page closed
            if any(err in tool_output for err in [
                'context or browser has been closed',
                'Execution context was destroyed',
                'Target page'
            ]):
                logger.info("Detected closed browser context; attempting recovery...", color='yellow')
                page = self.proxy.ensure_page()
                self.scanner = Scanner(page)
                logger.info("Browser context recovered.", color='green')
